...
function getBindPath(serverless, servicePath) {
  ...
  const startsWithMnt = baseBindPath.startsWith('/mnt/')
  const startsWithSingleLetter = baseBindPath[0] === '/' && baseBindPath[2] === '/'

  if (process.platform !== 'win32' && !isWsl) {
    return servicePath;
  } else if (isWsl && !startsWithMnt && !startsWithSingleLetter) {
    return servicePath
  }
  ...
  } else if (startsWithSingleLetter) {
    // gitbash "/c/users/..."
