...
function getSlimPackageCommands(options, targetRequirementsFolder){
  const stripCmd = [
    `&& find ${dockerPathForWin(options, targetRequirementsFolder)} -name "*.so" -exec strip {} ;`
    , `&& find ${dockerPathForWin(options, targetRequirementsFolder)} -name "*.py[c|o]" -exec rm -rf {} +`
    , `&& find ${dockerPathForWin(options, targetRequirementsFolder)} -type d -name "*tests*" -exec rm -rf {} +`
    , `&& find ${dockerPathForWin(options, targetRequirementsFolder)} -type d -name "__pycache__*" -exec rm -rf {} +`
    , `&& find ${dockerPathForWin(options, targetRequirementsFolder)} -type d -name "*.dist-info*" -exec rm -rf {} +`
  ];
  return stripCmd
}
