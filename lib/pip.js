...
function installRequirements(targetFolder, serverless, options) {
  ...
  if (options.dockerizePip) {
    ...
    let bindPath = dockerPathForWin(getBindPath(serverless, targetFolder));
    
    serverless.cli.log(
      `Using bindPath: ${bindPath}`
    );

    // Check for /.dockerenv
    serverless.cli.log(
      `Detecting Docker-in-Docker...`
    );
    
    const res = spawnSync('docker', ['inspect',process.env.HOSTNAME]);
    if (res.error) {
      throw res.error;
    }
    result = JSON.parse(res.stdout)[0]['GraphDriver']['Data']['MergedDir'];
    serverless.cli.log(`Found docker-in-docker volume mounted from: ` + JSON.stringify(result))
    serverless.cli.log(process.cwd())
    bindPath = result + process.cwd()
    
    // Works in Kubernetes...
    // cat /proc/self/cgroup | grep "systemd" | sed s/\\//\\n/g  | tail -1
    ...
    if (options.useDownloadCache) {
      ...
      let windowsized = getBindPath(serverless, downloadCacheDir);
      windowsized = result + windowsized;      // And now push it to a volume mount and to pip...
