...
function injectRequirements(requirementsPath, packagePath, options) {
  ...
  return fse.readFileAsync(packagePath).then(
    (buffer) => JSZip.loadAsync(buffer)
  ).then(
    (zip) => BbPromise.resolve(
      glob.sync([path.join(requirementsPath, '**')], {mark: true, dot: true})
    ).map(
      (file) => [file, path.relative(requirementsPath, file)]
    ).filter(
      ([file, relativeFile]) => !file.endsWith('/') &&
                                !relativeFile.match(/^__pycache__[\\/]/) &&
                                !noDeploy.has(relativeFile.split(/([-\\/]|\.py$|\.pyc$)/, 1)[0])
    ).map(
      ([file, relativeFile]) => zipFile(zip, relativeFile, fse.readFileAsync(file))
...
function moveModuleUp(source, target, module) {
  ...
  return fse.readFileAsync(source).then(
    (buffer) => JSZip.loadAsync(buffer)
  ).then(
    (sourceZip) => sourceZip.filter((file) => file.startsWith(module + '/'))
  ).map(
    (srcZipObj) => zipFile(
      targetZip, srcZipObj.name.replace(module + '/', ''),
...
function injectAllRequirements() {
  ...
  if (this.serverless.service.package.individually) {
    return BbPromise.resolve(
      ...
      (func) => (func.runtime || this.serverless.service.provider.runtime).match(/^python.*/)
    ).map(
      (func) => {
        if (!get(func, 'module')) {
          set(func, ['module'], '.');
      ...
      (func) => {
        if (func.module !== '.') {
          ...
          return moveModuleUp(artifact, newArtifact, func.module)
            .then(() => func);
        } else {
          return func;
        }
