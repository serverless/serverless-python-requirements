...
function addVendorHelper() {
  if (this.options.zip) {
    if (this.serverless.service.package.individually || !this.isPythonService) {
  ...
  if (this.options.zip) {
    if (this.serverless.service.package.individually || !this.isPythonService) {
      return BbPromise.resolve(this.targetFuncs)
        ...
        .then(funcs => {
          funcs.forEach(f => f.package.include.push(`${f.module}/.requirements.zip`)); 
          return uniqBy(funcs, f => f.module);
        })
        .map(f => {
          this.serverless.cli.log(
