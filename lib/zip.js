  ...
  if (this.options.zip) {
    if (this.serverless.service.package.individually) {
      return BbPromise.resolve(
        ...
        (functions) => uniqBy(functions, (func) => func.module)
      ).map(
        (f) => {
  ...
  if (this.options.zip) {
    if (this.serverless.service.package.individually) {
      ...
      return addTree(new JSZip(), '.serverless/requirements')
        .then((zip) => writeZip(zip, path.join(this.servicePath, '.requirements.zip')));
    }
  }
}
